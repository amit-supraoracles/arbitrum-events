...
async function response() {
  try {
    const outputFileName = 'responseData.json';
    let output = {};
    let nonceReached = false;

    for (let fromBlock = 1732742; fromBlock <= 11732742 && !nonceReached; fromBlock += 10000) {
      ...
      for (let loop = 0; loop < result.length && !nonceReached; loop++) {
